<template>
    <div class="infoDiploma">
        <h1 class="text text__heading">Courses that I've taken</h1>
        <p class="text text__content">
            This is the courses that I've taken. 
            They taught me a lot and with the help of the curse staff I was able to ask questions and get answers.  
            Of course they only covered their subjects so mostly I was self learning from the internet as well. 
            The courses gave me the basics of some thechnologis but all the issues and bugs that came along the way, I needed to resolve on my own.
        </p>
    </div>
</template>

<style lang="scss">
    .infoDiploma {
        grid-column: 3 / 5;
        grid-row: 10 / 11;
    }
</style>